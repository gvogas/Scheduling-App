...
│   ├── search/                      search_tokens.dart (searchIndexTokens — the prefix-and-substring token vocabulary the app WRITES into `clients.searchTokens` and `appointments.historySearchScopes`; query tokenizing moved server-side 2026-09-04, so `searchQueryTokens` has no `lib/` caller any more and survives only as the shared-example mirror reference for `functions/search_tokens.js`, which is what the server actually QUERIES with. `searchIndexTokens` is hand-mirrored by `functions/search_tokens.js` too, with worked examples shared value-for-value between `test/core/search/search_tokens_test.dart` and `functions/__tests__/search_tokens.test.js`, because a divergence is a search that silently returns nothing. Two ordering rules are load-bearing under the 240-token cap: each word emits its WHOLE token before any prefix, and the text and phone runs are interleaved so a long name cannot starve the phone out)
...
│   ├── app_bars/                    app_top_bar (AppTopBar — the shared header every screen uses; NOT an AppBar since 2026-09-11: it sits on the page colour with a displayLarge title, optional count, ghost controls and a pill search, so it re-supplies the system overlay style, the safe-area inset and end-drawer suppression itself, and its preferredSize is an upper bound reserved at the 2.2 text-scale cap rather than the rendered height; slims in landscape), app_header_pair (AppHeaderPair — Calendar pill + menu)
│   ├── primitives/                  app_avatar (contrast-aware initials circle), app_back_button (shared leading back arrow), busy_button_icon (spinner-or-icon slot for *.icon buttons), fade_in_item, ghost_control (GhostControl — the one owner of the ghost tile: a 38px painted `Ink` inside a 48px InkWell, so the tap floor is the gesture area and not just reserved layout; owns kGhostTile/kGhostTapTarget), scroll_to_top_button (ScrollToTopButton) over floating_pill (FloatingPill — the animated floating shell it shares with the calendar's TodayPill), name_initials (shared initials derivation), quick_action_button (QuickActionsRow + QuickActionButton — tinted Call/Email/Directions tiles), section_label (uppercase mini-header)
...
│   ├── fields/                      address_autocomplete_field, attached_dropdown (AttachedDropdown + AttachedDropdownRow — the suggestion list attached under a text field, for the address field AND the client picker. Extracted 2026-09-07 from two copies, the second carrying a comment asserting it matched the first; **it owned the CHROME ONLY until 2026-09-12**, so the two rows stayed different — the client's hand-built and divided, the address's a bare `ListTile(dense: true)` with no dividers and its own rhythm — and one form showed two controls doing one job. The row is shared now, and three things in it are load-bearing: the panel paints its own `palette.sheetRow` fill and `cardStyle.pillShadow` (it floats OVER the form and with no fill borrowed the sheet colour, separated in dark only by a 6%-white hairline), the row holds the 48pt tap floor itself (the client row painted ~36), and the trailing chevron is the whole affordance — the `tapTargetSize.shrinkWrap` "Attach" TextButton that sat INSIDE the row's own InkWell was a second, smaller target for one action, and both it and its word were dropped 2026-09-12 by owner call, taking `clients_attach` out of both ARBs), labeled_text_field (built-in shake + animated error row), app_search_bar, clear_text_button (ClearTextButton — the one clear-"x" suffix), form_helpers
...
The clients list uses `infinite_scroll_pagination` (v5): `ClientsRepository.fetchClientsPage` pages via a **value** cursor, so no `after` doc is re-fetched and Firestore types stay out of the domain layer. The cursor tuple FOLLOWS the chosen `ClientsSort` (name / most jobs / recently added) and its boundary cache is keyed `"<sort>:<docId>"` — a boundary captured under `name` would resume a `jobCount` query from a string. It does **not** stream — `clientsRefreshProvider` is bumped by every add/update/delete and the list listens to it to refresh. Committed search goes through the `searchClients` callable (see *Entity search* below); filtering the already-loaded pages is the instant fallback until it settles. The screen owns the chrome — ONE header row holding the round ghost Filter button (it opens the sheet; there are no filter chips since 2026-09-11 PM), the count sentence naming the active slice with a ✕ to clear it, and the sort control. `ClientsListView` has one caller and `ClientPicker` builds its own `AttachedDropdown` rows, so keeping the view chrome-free is design margin, not a live constraint. **Grouped mode (2026-09-11)** puts rows in cards under letter headings (`clients/domain/client_grouping.dart` — `letterGroupsOf` ONLY on the Type/Archived windows `sortClients` ordered by the folded display name; `singleGroupOf` for the unfiltered paged list, whose server order is the stored name — a person's phone — plus every non-name sort, search and building filter). A card is a `DecoratedSliver` around a `SliverList` so a group stays lazily built, which means the grouped path cannot use `PagedListView` and drives the pager through `PagedSliverPrefetch` + `PagedListFooter` (`clients/widgets/lists/paged_sliver_driver.dart`), shared with `AppointmentHistoryView`. Both memoize grouped rows through `RowCache<T>` (`clients/widgets/views/row_cache.dart`), cached at the SOURCE because `PagingState.items` re-flattens its pages on every access and an identity-keyed memo at the consumer never hits. A filtered list honours the sort by sorting the bounded window it already holds in Dart (`sortClients`, memoized) — no new reads or indexes; search stays relevance-ranked and ignores sort. Appointment photos display through `AppointmentImageCarousel` (`smooth_page_indicator`) in read-only mode; editing keeps the `PhotoPickerSection` thumbnail strip. New images are chosen via `pickAppointmentImages` (Camera gated by `MediaPermissionService`, Gallery via the OS photo picker).
...
- **The suite runs in UTC**, so a DST fix (e.g. `calendarDaysBetween`) has no
...
cache and reports ~50 suites failing to run on `Cannot find module
'firebase-admin/firestore'`, which looks like 44 broken tests and is nothing of
the kind. **The runner's count is the only one that settles whether the
