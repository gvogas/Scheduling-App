...
| Doc | State |
|---|---|
| `2026-09-11-four-bug-fixes.md` | **ALL FOUR BUILT 2026-09-11 — nothing deployed, no prod script run.** Four fixes: one search bar in the Add Appointment client picker, cancelled jobs excluded from `clients.jobCount`, the agenda's collapsed Done row restored to ~90 px, and the Clients filter path finally honouring the sort. Issues 1/3/4 are Dart-only and complete. **Issue 2's code is done but its DEPLOY is not, and still needs explicit authorization:** the new `appointments (clientId, status, dayIndex)` composite must be `READY` before the function deploys (the trigger is `retry: true` and rethrows, so a missing index is a redelivery loop), then `recount-client-jobs.js` — dry run, then live — as a release prerequisite. Verified at build time: analyzer clean, **3626 flutter**, **1896 jest / 88 suites**, eslint clean. |
...
| `2026-09-12-open-followups.md` | **OPEN — five items, none started.** The 2026-09-12 session's deliberate non-actions, each blocked on a different thing: the split container vocabulary on the appointment form (an owner decision — it IS the declined Option B), the street/city address split (a functions deploy, because the Places field mask omits `structuredFormat`), device verification of both rebuilt dropdowns (macOS Accessibility permission — synthetic clicks cannot drive `TextButton`/`CupertinoSwitch`), four CRLF Dart files (a commit boundary — `text=auto` will never normalize them and a text-mode edit already produced a lossy 1080-line diff once), and Phase 3's device pass. Written because a decision that lives only in a conversation gets re-litigated. |
