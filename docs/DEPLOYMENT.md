...
| 2026-09-19 16:37Z | `608b817a` | functions, firestore:rules, storage | 29 (unchanged) | **Month-end overdue review rider + Wave Phase 4 (`ed59a6a5`).** Rules and storage rules unchanged since `38c8225b`; they rode along as no-ops. No export change: 29 → 29, diffed BY NAME against `functions/index.js` and the live list before and after, so no deletion prompt and no failure-policy prompt; all 29 reported `Successful update operation`. `waveSetImportSchedule` stays deployed as the accepted no-op (Task 12.4); deleting it is a separate §4a deploy once 1.61.0 ages out. Pre-flight: eslint clean, jest **1971 / 95 suites** passing. Quiet-days guard for Phase 4 checked first: no WARNING+ on the Wave functions since the 2026-09-13 02:07Z deploy besides its own rollout probes. Post-deploy ERRORs were only the rollout health-probe `Invalid request` GETs (two per callable, 16:37:15–30Z). No index change, no prod script. **Still owed:** drop the automatic-import cadence wording from `docs/legal/` and republish privacy + terms to `es-pro-legal` (Task 12 note); the app build (step 4) carrying month-end, live map and the picker changes; after it ships, turn the month-end switch on for Paul. |
| 2026-09-19 ~19:45Z | `bec23b85` | functions, firestore:rules, storage | 29 (unchanged) | **Audit 2026-09-19 backend half (B1 + Wave log sanitizing).** Only `functions/` changed since `608b817a`: `waveUpsertCustomer` now clears a stale `blocked` verdict when a client write reverts its mapped fields to `wave.lastSyncedHash` (`isBlockedRevertToSynced` + `clearStaleBlock`; the clearing write sets `syncState: synced`, so the re-fire exits on both predicates), and `sync_run.js` logs Wave errors through `sanitizeError`/`describeWaveError`. `firestore.rules`, `storage.rules` and `firestore.indexes.json` unchanged; the CLI skipped both rules uploads as already current. Pre-flight: eslint clean, jest **1986 / 95 suites** passing. Run `--non-interactive`, env vars cleared and echoed empty in the same PowerShell invocation; no deletion, secret or retry prompt. All 29 `Successful update operation`. **Verified by NAME:** 29 live = 29 `exports.` in `functions/index.js`, zero missing, zero orphans. Post-deploy logs 19:35Z → 19:46Z at WARNING+: **empty**. The B1 path has not processed a real event yet, so it is unproven in prod. No index change, no prod script. **Still owed:** the `docs/legal/` cadence wording is already removed in `bec23b85`, so only the republish of privacy + terms to `es-pro-legal` remains; then the app build (step 4); then the month-end switch for Paul. |
